...
int ImageRecify(std::string &base64_data, std::string &result, std::string imgType)
{
  double degree;
  Mat src = BaseToMat(base64_data);
  if (src.empty())
  {
  ...
  result = MatToBase64(dst, imgType);
  return success;
}

// 根据边框来矫正图像，适合身份证银行卡文字不多的东西
int GetContoursPic(std::string &base64_data, std::string &result, std::string imgType)
{
  Mat srcImg = BaseToMat(base64_data);
  if (srcImg.empty())
  {
    // cout << "srcImg.empty()";
    return image_empty;
  ...
  result = MatToBase64(image, imgType);
  // imwrite("./output/temp/aa." + imgType, BaseToMat(result));
  return success;
}

// 自动透视变换矫正，使用于图像畸变
int CornerCheck(std::string &base64_data, std::string &result, std::string imgType) {
  Mat image = BaseToMat(base64_data);
  if (image.empty())
  {
    // cout << "srcImg.empty()";
    return image_empty;
